...
fun composeRunoutPdf(
    ...
    // ONE shared structure (`geom/ProfileFeatureSpans.kt`) with this sheet's WRITABLE
    // floors. Stored spec: tapers/liners/threads read identically from the resolved copy,
    // and the keyway pins must come from stored bodies anyway.
    ...
    // is untouched. The bolt count is the coupling bolt slot's — the first SEAM row, since a
    // cross-drilled hole is a bolt through the shaft, not a flange bolt on the face — and no
    // seam row authored means the plain two-circle face the hand sketch shows at minimum.
    if (drawFace) {
        drawCouplingFace(
            c = c,
            cx = couplingCx,
            ...
            boltCount = spec.couplerBoltSlots.firstOrNull { it.holeStyle == BoltHoleStyle.SEAM }?.count ?: 0,
            outline = outline,
            dim = dim,
            text = text,
...
internal fun drawShaftProfile(
    ...
    drawBodyRunsWithBreaks(
        c, spec.bodies, cy, xAt, rPx, outline, geomRect,
        fill = bodyFill,
        truePtPerMm = truePtPerMm,
